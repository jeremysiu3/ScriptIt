...
class Encoder(nn.Module):
    def __init__(self, input_size, embedding_dim, hidden_dim, dropout=0.5):
...
class Decoder(nn.Module):
    def __init__(self, output_size, embedding_dim, hidden_dim, dropout=0.5):
...
_output_char_to_idx = None
_output_idx_to_char = None
_device = None

def load_model():
    """Load the trained Mandarin seq2seq model (only once)"""
    global _model, _input_char_to_idx, _output_char_to_idx, _output_idx_to_char, _device
    ...
    if _model is None:
        ...
        _output_char_to_idx = vocabs['output_char_to_idx']
        ...
        checkpoint = torch.load('models/mandarin_seq2seq_model.pth', map_location=_device)
    ...
    return _model, _input_char_to_idx, _output_char_to_idx, _output_idx_to_char, _device

def translate_text(text, max_length=25):
    """Convert pinyin to Chinese using ML"""
    model, input_char_to_idx, output_char_to_idx, output_idx_to_char, device = load_model()
    ...
    with torch.no_grad():
        ...
        for idx in trg_indices[1:]:
            idx_str = str(idx)
            if idx == output_char_to_idx.get('<EOS>', -1):
                break
            if idx == output_char_to_idx.get('<PAD>', -1):
                continue
            char = output_idx_to_char.get(idx_str, '')
            result.append(char)
...
def convert_mandarin(text):
    """
    Convert pinyin to Chinese - exact match dictionary lookup, 
    otherwise use ML for entire input
    """
    from mappings.mandarin_common_words import MANDARIN_PHRASES
    
    text_lower = text.lower().strip()
    
    # EXACT MATCH ONLY: If entire input is in dictionary, use it
    if text_lower in MANDARIN_PHRASES:
        return MANDARIN_PHRASES[text_lower]
    
    # Otherwise: Use ML for the entire input
    return translate_text(text_lower)