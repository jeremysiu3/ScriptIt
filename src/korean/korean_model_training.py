...
class KoreanDataset(Dataset):
    def __init__(self, data, input_char_to_idx, output_char_to_idx, max_input_len=30, max_output_len=15):
        self.data = data
    ...
    def __getitem__(self, idx):
        ...
        # Convert output (Korean) to indices with <SOS> and <EOS>
        output_text = example['korean'][:self.max_output_len - 2]  # Leave room for SOS/EOS
...
print("\nPreparing dataset...")
full_dataset = KoreanDataset(training_data, input_char_to_idx, output_char_to_idx)
...
# Start training
trained_model = train_seq2seq(model, train_loader, val_loader, 20)
...
}, 'models/korean_seq2seq_model.pth')

print("\n✓ Model saved to 'korean_seq2seq_model.pth'")