...
def load_model():
    ...
    if _model is None:
        checkpoint = torch.load('models/hindi_vowel_model.pth', map_location=torch.device('cpu'))
...
def predict_vowels_hybrid(text):
    ...
    from mappings.hindi_common_words import HINDI_COMMON_WORDS
    
    text_lower = text.lower().strip()
    
    # Check dictionary
    if text_lower in HINDI_COMMON_WORDS:
        return HINDI_COMMON_WORDS[text_lower]
    ...
    if len(words) > 1:
        ...
        for word in words:
            ...
            if word_lower in HINDI_COMMON_WORDS:
                result_words.append(HINDI_COMMON_WORDS[word_lower])
            else:
